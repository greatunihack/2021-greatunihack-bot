...
export default class Bot {
  ...
  async setup(server: string): Promise<HTTPResponse> {
    ...
    const guildManager: OAuth2Guild = guilds.find(
      (guild: OAuth2Guild) => guild.id == server
    );
    ...
    const staffRole: Role = await guild.roles.create({
      name: "Staff",
      color: "#01ACFF",
      hoist: true,
    ...
    const sponsorRole: Role = await guild.roles.create({
      name: "Sponsor",
      color: "#EB403B",
      hoist: true,
      mentionable: false,
    });
    const judgeRole: Role = await guild.roles.create({
      name: "Judge",
      color: "#32C744",
      hoist: true,
      mentionable: false,
    });
    const mentorRole: Role = await guild.roles.create({
      name: "Mentor",
      color: "#765CB1",
      hoist: true,
      mentionable: false,
    ...
    const announcementsCategory: CategoryChannel = await guild.channels.create(
      "Group Channels",
      { type: "GUILD_CATEGORY" }
    );
    const staffCategory: CategoryChannel = await guild.channels.create(
      "Staff Channels",
      { type: "GUILD_CATEGORY" }
    );

    await guild.channels.create("welcome", {
      parent: announcementsCategory,
      permissionOverwrites: [
        { id: staffRole, allow: ["SEND_MESSAGES"] },
        { id: everyoneRole, deny: ["SEND_MESSAGES", "EMBED_LINKS"] },
      ],
    });
    ...
    await guild.channels.create("sponsors", {
      parent: announcementsCategory,
      permissionOverwrites: [
        { id: staffRole, allow: ["SEND_MESSAGES"] },
        { id: sponsorRole, allow: ["SEND_MESSAGES", "EMBED_LINKS"] },
        { id: everyoneRole, deny: ["SEND_MESSAGES", "EMBED_LINKS"] },
      ],
    });
    await guild.channels.create("general", {
      parent: announcementsCategory,
      permissionOverwrites: [{ id: everyoneRole, deny: ["EMBED_LINKS"] }],
    });
    await guild.channels.create("help", {
      parent: announcementsCategory,
      permissionOverwrites: [{ id: everyoneRole, deny: ["EMBED_LINKS"] }],
    });
    ...
    await guild.channels.create("staff", {
      ...
      permissionOverwrites: [
        { id: staffRole, allow: ["VIEW_CHANNEL"] },
    ...
    await guild.channels.create("sponsors", {
      ...
      permissionOverwrites: [
        { id: staffRole, allow: ["VIEW_CHANNEL"] },
        { id: sponsorRole, allow: ["VIEW_CHANNEL"] },
        { id: everyoneRole, deny: ["VIEW_CHANNEL"] },
      ],
    });
    await guild.channels.create("judges", {
      parent: staffCategory,
      permissionOverwrites: [
        { id: staffRole, allow: ["VIEW_CHANNEL"] },
        { id: judgeRole, allow: ["VIEW_CHANNEL"] },
        { id: everyoneRole, deny: ["VIEW_CHANNEL"] },
      ],
    });
    await guild.channels.create("mentors", {
      parent: staffCategory,
      permissionOverwrites: [
        { id: staffRole, allow: ["VIEW_CHANNEL"] },
        { id: mentorRole, allow: ["VIEW_CHANNEL"] },
        { id: everyoneRole, deny: ["VIEW_CHANNEL"] },
      ],
    });
  ...
  async newTeam(
    ...
    const teamRole: Role = await guild.roles.create({
      name: options.name,
      mentionable: false,
    });

    const teamCategory = await guild.channels.create(options.name, {
      type: "GUILD_CATEGORY",
      permissionOverwrites: [
        { id: staffRole, allow: ["VIEW_CHANNEL"] },
        { id: mentorRole, allow: ["VIEW_CHANNEL"] },
        { id: teamRole, allow: ["VIEW_CHANNEL"] },
        { id: everyoneRole, deny: ["VIEW_CHANNEL"] },
      ],
    });
    await guild.channels.create(options.name, {
      parent: teamCategory as CategoryChannel,
    });
    await guild.channels.create(options.name, {
      type: "GUILD_VOICE",
      parent: teamCategory as CategoryChannel,
  ...
  async deleteTeam(server: string, team: string): Promise<HTTPResponse> {
    ...
    const categoryChannels: Collection<string, CategoryChannel> =
      channels.filter(
        (channel: AnyChannel) => channel.type == "GUILD_CATEGORY"
      ) as Collection<string, CategoryChannel>;
    const teamCategory: CategoryChannel = categoryChannels
      .filter(
        (channel: CategoryChannel) =>
          channel.permissionsFor(role).has("VIEW_CHANNEL") &&
          channel.name != "Group Channels" &&
          channel.name != "Staff Channels"
      )
      .map((x) => x)[0];
    const teamChannels: Collection<string, Channel> = channels.filter(
      (channel: AnyChannel) => channel.parent == teamCategory
    );
  ...
  async assignParticipant(
    ...
    const user: GuildMember = users.find(
      (user: GuildMember) => user.id == participant
    );
    const role: Role = roles.find((role: Role) => role.id == team);
